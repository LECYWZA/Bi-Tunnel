<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center bt-section-status">
      <div class="flex flex-col">
        <div class="flex items-center gap-2">
          ...
          <span class="font-bold text-lg bt-text">{{ t('tester.title') }}</span>
        </div>
        <span class="text-xs bt-text-muted mt-1">{{ t('tester.headerSubtitle') }}</span>
      </div>
    ...
    <el-empty v-if="profiles.length === 0" :description="t('tester.emptyDescription')" :image-size="80" />
      ...
      <el-card v-for="(profile, index) in profiles" :key="String(profile.id)" shadow="hover" class="bt-card h-full" body-class="flex flex-col h-full p-5">
        ...
        <div class="flex justify-between items-center mb-5 pb-4" style="border-bottom: 1px solid var(--bt-border);">
          <div class="flex items-center gap-3 flex-1">
            <el-input v-model="profile.name" :placeholder="t('tester.profileNamePlaceholder')" size="small" class="w-48" @change="saveProfiles" />
          </div>
          <div class="flex gap-2 items-center">
            <el-button type="primary" plain :icon="CopyDocument" size="small" @click="copyProxyUrl(profile)">{{ t('tester.copyLink') }}</el-button>
            <el-button type="danger" plain :icon="Delete" size="small" @click="removeProfile(index)">{{ t('common.delete') }}</el-button>
          </div>
        </div>
        ...
        <el-form label-position="top" size="default" class="flex-1">
          <el-row :gutter="20">
            <el-col :xs="24" :sm="12">
              <el-form-item :label="t('tester.protocolLabel')">
                <el-select v-model="profile.type" class="w-full" @change="saveProfiles">
                  <el-option :label="t('tester.protocolHttp')" value="http" />
                  <el-option :label="t('tester.protocolSocks5')" value="socks5" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :xs="24" :sm="12">
              <el-form-item :label="t('tester.serverIpLabel')">
                <el-input v-model="profile.host" :placeholder="t('tester.serverIpPlaceholder')" @change="saveProfiles" />
              </el-form-item>
            </el-col>
            <el-col :xs="24" :sm="12">
              <el-form-item :label="t('tester.serverPortLabel')">
                <el-input-number v-model="profile.port" :min="1" :max="65535" class="w-full" controls-position="right" @change="saveProfiles" />
              </el-form-item>
            </el-col>
            <el-col :xs="24" :sm="12">
              <el-form-item :label="t('tester.targetUrlLabel')">
                <el-input v-model="profile.targetUrl" :placeholder="t('tester.targetUrlPlaceholder')" @change="saveProfiles" />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <div class="p-4 rounded-lg mb-4" style="background: var(--bt-input-bg); border: 1px solid var(--bt-border);">
            <el-row :gutter="20">
              <el-col :xs="24" :sm="12">
                <el-form-item :label="t('tester.usernameLabel')" class="mb-0">
                  <el-input v-model="profile.username" :placeholder="t('tester.authPlaceholder')" :prefix-icon="User" @change="saveProfiles" />
                </el-form-item>
              </el-col>
              <el-col :xs="24" :sm="12">
                <el-form-item :label="t('tester.passwordLabel')" class="mb-0 mt-4 sm:mt-0">
                  <el-input v-model="profile.password" :placeholder="t('tester.authPlaceholder')" show-password :prefix-icon="Lock" @change="saveProfiles" />
                </el-form-item>
              </el-col>
            </el-row>
          </div>
        ...
        <div v-if="profile.logs && profile.logs.length > 0" class="mt-5 rounded-lg overflow-hidden" style="border: 1px solid var(--bt-border);">
          <div class="flex justify-between items-center p-3" style="background: var(--bt-input-bg); border-bottom: 1px solid var(--bt-border);">
            <span class="font-mono text-xs flex items-center gap-2 bt-text-secondary">
              <el-icon><Monitor /></el-icon> {{ t('tester.logOutput') }}
            </span>
            <el-tag
              ...
              size="small"
            >
              {{ profile.testResult === true ? t('tester.testSuccessTag') : (profile.testResult === false ? t('tester.testFailedTag') : t('tester.testWaitingTag')) }}
            </el-tag>
          </div>
...
const copyProxyUrl = async (profile) => {
  try {
    ...
    ElMessage.error(t('tester.copyFailed'));
  }
};
...
const runTest = async (profile) => {
  if (!profile.host || !profile.port || !profile.targetUrl) {
    ElMessage.warning(t('tester.formRequired'));
    return;
  }

  profile.testing = true;
  profile.logs = [t('tester.requestStartLog')];
  ...
  try {
    ...
    if (data.success) {
      ...
      ElMessage.error(t('tester.profileTestFailed', { name: profile.name }));
    }
  } catch (err) {
    if (err.name === 'AbortError') {
      profile.logs.push(t('tester.testCanceledLog'));
      profile.testResult = false;
      ElMessage.info(t('tester.testCanceled'));
    } else {
      profile.logs.push(t('tester.requestFailedLog', { msg: err.message }));
      profile.testResult = false;
      ElMessage.error(t('tester.requestFailed'));
    }
  } finally {
