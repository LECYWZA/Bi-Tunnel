...
```aiignore
审计日志显示的链路还是不对，而且系统代理和虚拟网卡开哪一个，审计日志经常看不到日志，我浏览器明明一直刷新对某个网站的访问，审计日志也没增加日志，而且我的代理链明明是两个代理，连接起来，但是链路一直是显示一个代理，还有就是“分流规则表配置”的规则卡片和要支持排序，选中的多个规则，排在前面的先命中先生效，转发至选择已经选择的也要支持拖动排序，两个都改造成就类似设置代理线路的那种ui拖动的排序，原来只是单条选择多个select，不满足我的要求
```