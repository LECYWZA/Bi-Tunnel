...
```待实现
服务端顶部也要加入卡片说明。然后“服务端连接池”的服务停用后，客户端管理还显示在线，这个就不对了，还有新增一个服务端刷新后又没了，根本无法保存。客户端管理的在线离线我要的是实时更变，而不用刷新浏览器，ws体验才最好；还有代理池的按钮颜色比文字还亮，不管是暗夜还是白天主题，文字都不够明显，这样是不行的，实时流量审计的卡片暗夜模式还显示白色也不行，然后测速台的测试配置的“测试配置”的文字可以去掉，都被挤压换行了，这种状态类的如果文件不好管理可以引入轻量化的数据库，但是配置我还是坚持json的方式
```